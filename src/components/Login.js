...
function Login({toggleLoginContainer, cookies, setSessionProfilePic, BASE_URL_BACK, setToggleLoginContainer, setSessionUsername, setIsLoggedIn}){
    ...
    function handleSuccessLogin(message){
        ...
        setSessionProfilePic(message.avatar_path)
        setErrorMessage("")
            ...
            <div className={toggleLoginContainer? "userLoginContainer fade-in" : "userLoginContainer"}>
                ...
                <form className="userRegisterForm" onSubmit={handleOnLoginSubmit}>
                    ...
                    <div className={toggleRegister?"userCFPW fade-in" : "userCFPW hidden"}>
                        <label>Email: </label> 
                        <input type="email" name="useremail" value={formData.useremail} onChange={handleOnChange}/> 
                    </div>
                ...
                <img className={toggleRegister? "showPasswordRegister" : "showPassword"} src={showPassword? eyeballIcon : eyeballClosedIcon} alt="eyeball" onClick={()=> setShowPassword(!showPassword)}/>
            </div>
        </div>
    )
}
