...
function TVContainer({ isLoggedIn, cookies, BASE_URL_BACK, onLogOut, setOnLogOut}){
    ...
    const [movieID, setMovieID] = useState("tv/popular")         //belongs to Header.js, moved up for watchlist cards not populating header with full details
    ...
    `${apiPrefixURL}discover/tv?api_key=${apiKey}&page=${pageNumber}${yearOrGenreSuffix}` : 
        ...
        if(!isWatched){                                                 // not yet added to WL so add to WL
            ...
            switch(watchListArray.length){                              // checks to see if movie is allowed to be added into watchlist
                case 0:{                                                // handles adding the first movie into an empty watchlist    
                    sendWLDataToDB(dataToSend, `${BASE_URL_BACK}/watchlist_cards`)
                    break;
                }
